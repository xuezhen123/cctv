				...
				<tr>
	           	...
	           	<tr>
		           	...
		           	<td colspan="3" id="roleTD">
		           		<!-- 使用checkbox展示所有的角色数据到页面中 -->
		           		<script type="text/javascript">
		           			$(function(){
		           				//页面加载完成后，发送ajax请求，查询所有的角色数据，在页面中展示为checkbox
		           				$.post("roleAction_listajax.action",function(data){
		           					if(data.length > 0){
		           						for(var i=0;i<data.length;i++){
		           							var id = data[i].id;
		           							var name = data[i].name;
		           							$("#roleTD").append('<input id="'+id+'" type="checkbox" value="'+id+'" name="roleIds"><label for="'+id+'">'+name+'</label>');
		           						}
		           					}
		           				});
		           			});
